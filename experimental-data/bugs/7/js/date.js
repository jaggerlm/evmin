...
(function () {
  ...
  g.hms = _.cache(_.sequence([g.H,
  g.mm,
  g.ss], g.timePartDelimiter));
  g.t = _.cache(_.process(g.ctoken2('shortMeridian'), t.meridian));
  g.tt = _.cache(_.process(g.ctoken2('longMeridian'), t.meridian));
  g.z = _.cache(_.process(_.rtoken(/^(\+|\-)?\s*\d\d\d\d?/), t.timezone));
  g.zz = _.cache(_.process(_.rtoken(/^(\+|\-)\s*\d\d\d\d/), t.timezone));
  g.zzz = _.cache(_.process(g.ctoken2('timezone'), t.timezone));
  g.timeSuffix = _.each(_.ignore(g.whiteSpace), _.set([g.tt,
  g.zzz]));
  ...
  g._formats = g.formats(['yyyy-MM-ddTHH:mm:ss',
  'ddd, MMM dd, yyyy H:mm:ss tt',
  'ddd MMM d yyyy HH:mm:ss zzz',
  'd']);
  g._start = _.process(_.set([g.date,
  g.time,
  g.expression], g.generalDelimiter, g.whiteSpace), t.finish);
  g.start = function (s) {
    try {
