...
(function (sandbox) {
    ...
    var ignoreInst = Config.ignoreInst;
...
//        N_LOG_FUNCTION_ENTER_NORMAL = 18,
	...
	var logCondEnterFunName = astUtil.JALANGI_VAR + ".Ce";
	var logCondExitFunName = astUtil.JALANGI_VAR + ".Cr";
...
//        StatCollector.suspendTimer("replace");
    ...
    function wrapMethodCall(node, base, offset, isCtor) {
		if(isIgnore(base.name,null,node.callee,node) || base.ignoreSubAst){
			return node.callee;
		}
		printIidToLoc(node);
    ...
    function wrapFunCall(node, ast, isCtor) {
        if(isIgnore(null,node.callee.name,ast,node)){
			return ast;
		}
		printIidToLoc(node);
        var ret = replaceInExpr(
    ...
    function wrapGetField(node, base, offset) {
		if(isIgnore(base.name,null,node) || base.ignoreSubAst){
			node.ignoreSubAst = true;
			return node;
		}	
	...
	function isIgnore(base,offset,ast,node){
		var name = (base||'') + (offset||'');
		if(ignoreInst.hasOwnProperty(name)){
			var ign = ignoreInst[name];
			if(ign.code && node && node.type == 'CallExpression'){
				for(var i=0;i<ign.code.length;i++){
					node.arguments[i] = wrapFunArg(node.arguments[i]);
				}
			}
			ast.ignoreArg = (ign.args!=false);
			ast.ignoreSubAst = true;
			return ast;
		}
	}
    function wrapRead(node, name, val, isReUseIid, isGlobal, isPseudoGlobal) {
		if(isIgnore(null,name.value,node)){
			return node; 
		}
...
//    }
    ...
    function wrapReadWithUndefinedCheck(node, name) {
       	if(isIgnore(null,node.name,node)){
			return node;
		} 
        var ret;
        if (name !== 'location') {
...
//        );
	...
	function wrapFunArg(ast){
		return wrapCode(ast);
	}
	function wrapCode(ast){
		printIidToLoc(ast);
        var ret = replaceInExpr(
...
//    }
    ...
    function createCallInitAsStatement(node, name, val, vartype, lhs) {
        printIidToLoc(node);
        var ret;

        if (vartype == 'arg' || vartype == 'var'){
            var isArgumentSync = (vartype == 'arg');
		...
		}else if(vartype == 'defun'){
            ret = replaceInStatement(
                logInitFunName + "(" + RP + "1, " + RP + "2, " + RP + "3, " + false + ", false)",
                getIid(),
                name,
                val
            );
            ...
            for (var name in scope.vars) {
                if (HOP(scope.vars, name)) {
                    ...
                    if (scope.vars[name] === "defun") {
                    	...
                    	val = wrapLiteral(ident, ident, N_LOG_FUNCTION_LIT);
						ret = ret.concat(createCallInitAsStatement(node,
                    ...
                    if (scope.vars[name] === "arg" || scope.vars[name] === "var") {
                        val = ident;
						ret = ret.concat(createCallInitAsStatement(node,
                        	createLiteralAst(name),
	                        val,
...
//            return ret1;
        ...
        "CallExpression":function (node) {
			node.callee = instrumentCall(node, false);
            return node;
        },
...
//                var ret = prependScriptBody(node, body);
        ...
        "IfStatement":function(node){
			node.test = wrapConditional(node.test, node.test);
			wrapCondBlock(node.consequent,node.test.iid);
			wrapCondBlock(node.alternate,node.test.iid);
			return node;
		},
	...
	function wrapCondBlock(block,tiid){
		if(block && block.body && block.body.length>0){
			var first = block.body[0];
			var last = block.body[block.body.length-1];
			printIidToLoc(first);
			var ce  = replaceInStatement(logCondEnterFunName+'('+RP+'1,'+RP+'2)',getIid(),tiid)[0];
			block.body.unshift(ce);
			transferLoc(ce, first);
			printIidToLoc(last);
			var cr = replaceInStatement(logCondExitFunName+'('+RP+'1,'+RP+'2)',getIid(),tiid)[0];
			block.body.push(cr);
			transferLoc(cr, last);
		}
	}
...
//        console.time("transform")
        ...
        for (var i = 0; i < len; i++) {
            newAst = astUtil.transformAst(newAst, visitorsPost[i], visitorsPre[i], astUtil.CONTEXT.RHS,null,null,null,true);
...
//                StatCollector.suspendTimer("generate");
	...
	function getCodeLines(exp){
		var ast,lines=[];
		if(typeof exp ==='string'){
			ast = acorn.parse(exp,{sourceFile:true});
			body = ast.body;
		}else if(typeof exp ==='function'){
			ast = acorn.parse('('+exp.toString()+')');
			body = ast.body[0].expression.body.body;
		}
		for(var i=0;i<body.length;i++){
			var code = escodegen.generate(body[i]);
			lines.push(code);
		}
		return lines;
	};
...
//        console.timeEnd("save")
    ...
    if (typeof window === 'undefined' && (typeof require !== "undefined") && require.main === module) {
		...
		sandbox.getCodeLines = getCodeLines;
    }
