...
(function () {
    ...
    function ignoreSubAst(node) {
        return node.type === 'CallExpression' && node.callee.type === 'MemberExpression' &&
			...
			node.ignoreSubAst ||
			(node.type == 'CallExpression' && node.callee.type == 'MemberExpression' &&  node.callee.object.ignoreArg)||
			(node.type=='CallExpression' && node.callee.type=='Identifier' && node.callee.ignoreArg);
    }
     ...
     */
	var temp = {};
    function transformAst(object, visitorPost, visitorPre, context, noIgnore, wj_p,wj_k,wj_x) {
...
//            if (object.hasOwnProperty(key)) {
				...
				if (typeof child === 'object' && child !== null && key !== "scope" && (noIgnore || !ignoreSubAst(object))) {
					...
					object[key] = transformAst(child, visitorPost, visitorPre, newContext, noIgnore,object,key);
                }
//            }
        }
