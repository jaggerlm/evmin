...
(function(sandbox){
    ...
    function readInput(val, isAux, symbolOptional) {
		var concrete = getConcrete(val);
        var ret, type, fieldsOrdered, i, len;
        ...
        if (concrete === undefined) {
            val = getNextConcreteInput(idx,[val, [], [typeof concrete] , 0, false]);
        } else {
            val = getNextConcreteInput(idx,[val, [], [typeof concrete] , 0, true]);
        }
        ret = val[0];
        type = typeof concrete;

        ret = sandbox.analysis.invoke('makeConcolic',idx, val, getNextSymbol);
        ...
        sandbox.analysis.invoke('makeConcolicPost');
        //inputs[idx] = ret;
        return ret;
    }
